              ...
              {[
                ...
                <button key={f.value} onClick={() => setFilter(f.value)}
                  className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold transition
                    ${filter === f.value ? 'bg-emerald-500 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                  {f.label}
                </button>
              ))}
            </div>
                ...
                <div key={review._id} className="bg-white border border-gray-100 rounded-xl p-4">
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    {!review.hostReply?.text && (
                      <span className="text-xs bg-yellow-100 text-yellow-600 px-2 py-0.5 rounded-full font-semibold shrink-0">Needs reply</span>
                    )}
                  </div>

                  {/* Review text */}
                  <p className="text-xs text-gray-700 leading-relaxed mb-4">{review.comment}</p>

                  {/* Host reply */}
                  {review.hostReply?.text ? (
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                      <p className="text-xs font-bold text-emerald-600 mb-1">Your Response</p>
                      ...
                      <button
                        ...
                        className="text-xs text-emerald-500 hover:underline mt-2 font-medium"
                      >
                        Edit reply
                      </button>
                    </div>
